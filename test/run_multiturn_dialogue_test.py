...
def check_turn_assertions(turn: dict, turn_spec: dict) -> list[dict]:
    ...
    trace_text = json.dumps(turn.get("rag_trace", {}), ensure_ascii=False)
    ...
    if expected_action:
        ...
        if expected_action == "tool":
            action_passed = bool(tool_names)
            action_detail = f"期望工具动作，实际工具: {tool_names}"
        elif expected_action == "ask_clarification":
            clarification_markers = ["确认", "是不是", "你是想", "请先告诉", "哪道菜", "具体是"]
            action_passed = not tool_names and any(marker in full_text for marker in clarification_markers)
            action_detail = f"期望追问且不调用工具，实际工具: {tool_names}"
        elif expected_action == "offer_web_search":
            offer_markers = ["需要我帮你到网上搜一下吗", "需要我帮你联网搜索一下吗", "要不要我帮你联网搜"]
            action_passed = "web_search_tool" not in tool_names and any(marker in full_text for marker in offer_markers)
            action_detail = f"期望请求联网确认且不直接联网，实际工具: {tool_names}"
        elif expected_action == "answer":
            action_passed = bool(assistant_text.strip())
            action_detail = "期望直接回答"
        else:
            action_passed = False
            action_detail = f"未知 expected_action: {expected_action}"
    ...
    expect_pending_type = turn_spec.get("expect_pending_type")
    if expect_pending_type:
        assertions.append(dict(
            name="expect_pending_type",
            passed=str(expect_pending_type) in full_text,
            detail=f"期望 pending 类型: {expect_pending_type}",
        ))

    if turn_spec.get("resolves_pending"):
        assertions.append(dict(
            name="resolves_pending",
            passed=bool(tool_names) or bool(assistant_text.strip()),
            detail=f"期望本轮解决上一轮 pending，实际工具: {tool_names}",
        ))

    if turn_spec.get("expect_offer_web_search"):
        offer_markers = ["需要我帮你到网上搜一下吗", "需要我帮你联网搜索一下吗", "要不要我帮你联网搜"]
        assertions.append(dict(
            name="expect_offer_web_search",
            passed=any(marker in full_text for marker in offer_markers),
            detail="期望出现联网搜索确认话术",
        ))

    if turn_spec.get("expect_no_web_before_confirmation"):
        assertions.append(dict(
            name="expect_no_web_before_confirmation",
            passed="web_search_tool" not in tool_names,
            detail=f"确认前不应调用 web_search_tool，实际工具: {tool_names}",
        ))

    choice_prompt = turn.get("rag_trace", {}).get("choice_prompt") if isinstance(turn.get("rag_trace"), dict) else None
    if turn_spec.get("expect_choice_prompt"):
        assertions.append(dict(
            name="expect_choice_prompt",
            passed=isinstance(choice_prompt, dict),
            detail="期望 rag_trace.choice_prompt 存在",
        ))

    expect_choice_type = turn_spec.get("expect_choice_type")
    if expect_choice_type:
        actual_type = choice_prompt.get("type") if isinstance(choice_prompt, dict) else None
        assertions.append(dict(
            name="expect_choice_type",
            passed=actual_type == expect_choice_type,
            detail=f"期望 choice_prompt.type={expect_choice_type}, 实际={actual_type}",
        ))

    expect_choice_options = turn_spec.get("expect_choice_options", [])
    if expect_choice_options:
        actual_options = [str(item.get("key")) for item in choice_prompt.get("options", [])] if isinstance(choice_prompt, dict) else []
        missing_options = [key for key in expect_choice_options if key not in actual_options]
        assertions.append(dict(
            name="expect_choice_options",
            passed=len(missing_options) == 0,
            detail=f"期望选项: {expect_choice_options}, 实际: {actual_options}" + (f", 缺失: {missing_options}" if missing_options else ""),
        ))
...
澄清门规则：
1. 当用户语义不确定、菜名不稳定、正向/反向意图冲突、本地未收录但用户尚未授权联网时，assistant 主动追问或请求联网确认是合格行为。
2. 不要因为 assistant 没有立刻给出最终菜谱而自动判失败；必须先看该轮 expected_action 是否要求 ask_clarification 或 offer_web_search。
3. 如果 expected_action=ask_clarification，且 assistant 没有编造答案、没有调用工具、提出了可执行的确认问题，应判通过。
4. 如果 expected_action=offer_web_search，且 assistant 没有直接联网、明确请求用户确认联网，应判通过。
...
async def run_single_case(case: dict) -> dict:
    ...
    for turn_idx, turn_spec in enumerate(case["turns"]):
        user_text = _resolve_turn_user_text(turn_spec, turns_result)
        # 执行本轮
        turn_result = await run_single_turn(user_text, history)
        turn_result["user"] = user_text
...
def _resolve_turn_user_text(turn_spec: dict, turns_result: list[dict]) -> str:
    if "choose_option" not in turn_spec:
        return str(turn_spec["user"])

    if not turns_result:
        raise ValueError("choose_option 不能用于第一轮")

    choice_key = str(turn_spec["choose_option"])
    previous = turns_result[-1]
    trace = previous.get("rag_trace") if isinstance(previous.get("rag_trace"), dict) else {}
    prompt = trace.get("choice_prompt") if isinstance(trace, dict) else None
    if not isinstance(prompt, dict):
        raise ValueError(f"上一轮没有 choice_prompt，无法选择 {choice_key}")

    options = prompt.get("options") if isinstance(prompt.get("options"), list) else []
    option = next((item for item in options if isinstance(item, dict) and str(item.get("key")) == choice_key), None)
    if not option:
        raise ValueError(f"choice_prompt 中没有选项 {choice_key}")

    if option.get("custom"):
        custom_input = str(turn_spec.get("custom_input") or "").strip()
        if not custom_input:
            raise ValueError(f"选项 {choice_key} 是 custom，测试必须提供 custom_input")
        return custom_input

    send_text = str(option.get("send_text") or "").strip()
    if not send_text:
        raise ValueError(f"选项 {choice_key} 缺少 send_text")
    return send_text
